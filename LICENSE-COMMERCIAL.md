# Commercial License — Rotas Capitais

Copyright (C) 2025-2026 Gabriel Felipe Guarnieri. All rights reserved.

## Retroactive Clause

This license applies retroactively to all past commits and versions of this
repository, superseding any previously stated license.

## Overview

This software is primarily licensed under the **GNU Affero General Public
License v3.0 (AGPL-3.0)**. The AGPL-3.0 requires that any modified version of
this software, or any software that incorporates it, must also be released
under the AGPL-3.0 — including when the software is accessed over a network.

If the AGPL-3.0 terms are incompatible with your intended use, a **Commercial
License** is available that grants you the right to use, modify, and distribute
this software without the AGPL-3.0 obligations.

## When You Need a Commercial License

You need a commercial license if any of the following apply:

- You want to integrate this software into **proprietary or closed-source**
  products or services.
- You want to offer this software as part of a **SaaS, PaaS, or hosted
  service** without disclosing your source code.
- You want to distribute this software (or derivatives) **without the
  AGPL-3.0 copyleft requirement**.
- Your organization's legal or compliance policies **prohibit use of
  AGPL-licensed software**.

## What the Commercial License Grants

A commercial license provides:

- **No copyleft obligation** — use and modify the code without releasing your
  own source code.
- **Proprietary distribution** — ship the software in closed-source products.
- **SaaS deployment** — host the software as a service without network
  disclosure requirements.
- **Warranty and liability terms** — negotiable on a per-agreement basis.
- **Priority support** — available as an optional add-on.

## How to Obtain a Commercial License

To discuss commercial licensing terms, pricing, or volume arrangements, contact
the copyright holder:

**Gabriel Felipe Guarnieri**
- LinkedIn: [https://www.linkedin.com/in/oguarni/](https://www.linkedin.com/in/oguarni/)

## Important Notice

Using this software without either (a) complying with the AGPL-3.0 or
(b) obtaining a commercial license constitutes copyright infringement.

This document is informational and does not constitute a license grant. A
commercial license is effective only upon execution of a written agreement
between you and the copyright holder.
